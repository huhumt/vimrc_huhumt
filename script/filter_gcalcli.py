...
def update_event_dict(date_log: str) -> OrderedDict:
    ...
    for (
        ...
        if meeting_room := re.findall(re_meetint_room, location):
            event_attr_list = [meeting_room[0].strip()]
        elif location and not england_holiday:
            event_attr_list = [location]
        else:
        ...
        if attr := re.findall(re_attr, description):
            ...
            if not any([location, england_holiday]):
                event_attr_list.append(
                    re.sub("holidays ", "", calendar, flags=re.IGNORECASE))
...
def update_from_file(event_dict, days_later: int) -> dict | None:
    ...
    for today_event_dict in event_dict:
        ...
        if holiday := re.findall(re_holiday, event, re.IGNORECASE):
            name = holiday[0][0]
