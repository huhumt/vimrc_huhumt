...
IN_EVENT_COLOUR = (
    COLOUR_BG_YELLOW + "[*] {event}" + COLOUR_GREEN + "{url}" + COLOUR_OFF
)
...
def to_colour(day: str, header: str, event_dict: dict, colour_en: bool) -> str:
    ...
    out_list = [[HEADER_COLOUR.format(day=day, header=header)], [], [], []]
...
def update_event_dict(date_log: str, today_date: datetime) -> OrderedDict:
    ...
    month_b = [
        datetime.strptime(str(i).zfill(2), "%m").strftime("%b")
        for i in range(1, 13)
    ]
            ...
            for k, v in e_dict.items():
                for name, duration in re.findall(re_holiday, k, re.IGNORECASE):
                    if (left_day := int(duration) - days_later) > 0:
                        ...
                        file_holiday.update(
                            {f"{name.strip()} ({left_day} {day_label})": v}
                        )
    return file_holiday
...
if __name__ == "__main__":
    ...
    if (days_later := set_days_later(args.days_later, today_date)) == 0:
        days_later_key = to_date(today_date)
    else:
    ...
    try:
        with open(
            GCALCLI_FILENAME, "r", encoding="utf-8", errors="ignore"
        ) as f:
    ...
    if from_file_flag:
        ...
        event_dict = update_event_dict(
            remove_colour(sys.stdin.buffer.read().decode("utf-8", "ignore")),
            today_date,
        )
